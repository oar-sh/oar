...
| Steering mid-turn | Partial — continuation turns only (2026-09-25 audit) | The worker does **not** opt into the shared link's steering (no `getSteeringReady` / `getDeliveryHeld` / heartbeat `steeringState`), so during a *delivered* turn the link stays single-flight and a second message waits in the queue; `steerIntoActiveTurn` effectively runs only during a *continuation* turn (no delivery in flight) or after a socket reconnect — and there it is not gated on an open `ask_user` card (Claude hands such deliveries back as `steering-held`). Newer runtime bundles add a real acknowledgement the worker ignores: `user.message.delivery` (`idle` / `steering` / `queued`) plus `messageId` matching the id `send()` resolves (the worker discards it) and `turnId`, and an experimental `session.rpc.queue` (`pendingItems` with `inFlightSteeringCount`, `sendNow` → `{steered}`). Parity plan: `docs/plans/copilot-sdk-steering-background-parity.md` (local). Mechanics today: `steerIntoActiveTurn` sends with `mode: 'enqueue'` (live-probed: `immediate` does **not** preempt an in-flight model call, and the whole interaction closes with a single `session.idle`), adopts the row onto the running turn so the lease is renewed and the crash guard requeues both, and answers each row from its own prompt segment indexed by **send order** (`settleSteeredRows`; a row that never got a segment gets `STEERED_ROW_MERGED_NOTE` rather than a requeue that would run it twice). |
...
| Background-task gating | Implemented (shells, not "tasks") | `session.background_tasks_changed` is still useless — empty payload, ~23 per bash call, no id or state — so gating keys on **detached shells** tracked from tool events and `system.notification`'s typed `shell_detached_completed` / `shell_completed` kinds instead (`copilot-continuation-signals.mjs`). Capped by `getBackgroundTaskTimeoutMs()` (`COPILOT_SDK_RELAY_BACKGROUND_TASK_TIMEOUT_MS`, 30 min, `0` = unlimited). Worker `stop_background_task` controls are still logged and ignored and cards publish `stoppable: false` — but newer bundles expose an experimental `session.rpc.tasks` API (`list` / `getProgress` / **`cancel`**, `TaskAgentInfo` with `model`/`resolvedModel`/`executionMode`, `TaskShellInfo` with `command`) that could replace the event-scraped shell tracking and make cards stoppable (unverified live; whether `TaskShellInfo.id` equals the notification `shellId` is open). Background *subagents* (`subagent.started.executionMode`, `agent_completed` / `agent_idle` notifications) are not surfaced as task cards yet. |
