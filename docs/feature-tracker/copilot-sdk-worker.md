...
| Steering mid-turn | Implemented (2026-09-25, Claude parity) | The worker opts into the shared link's steering gate (`getSteeringReady` / `getDeliveryHeld`) and publishes the composer snapshot on its heartbeat (`steeringState()` with `supported: true`, `holdReason` ∈ question / compaction / delivery, and the un-steerable `cancellableIds`). Every send uses `mode: "immediate"` (probe 2026-09-25, runtime 1.0.88): mid-turn the runtime injects it at the next tool boundary as `user.message{delivery:"steering"}` — the row settles `kind='folded'` and is marked consumed at that acknowledgement — or, when the model is mid-stream, runs it right after as `delivery:"queued"` on its own row (`user.message.messageId` matches the id `send()` resolves; rows are attributed by id, never by text). Holds: an open `ask_user` / approval / elicitation card, a compaction (`session.compaction_start`…`_complete`, 10-min stale cap) and the window before the turn-opening send resolves hand a delivery back as `steering-held`. Steered prompts carry the hidden steer note (`shared/steer-note.mjs`). Stop = `rpc.interruptMainTurn({flushQueued:false})` (falls back to `session.abort()`): the stopped row is left to the relay's abort control, other pushed rows settle `kind='stopped'` (Resend), and anything of ours still in the queued lane is removed first. Un-steer (narrowed after the live check, 2026-09-25): immediate sends wait in the runtime's steering lane, which has no ids (`pendingItems.steeringMessages` is text only), so only the NEWEST waiting steer is recallable — `rpc.queue.removeMostRecent` (LIFO across both lanes, probe-verified); a row in the queued lane goes by its stable id (`removeAt`). The worker reports it as `cancellableIds` (our newest unconsumed steer, ≥1 steer waiting, empty queued lane); `cancel_pushed_message` → remove → `POST /api/queue-cancelled`. Race guards: no newest-recall while any steer `send()` is unanswered (`steerSendsInFlight`), "newest" by send order (`sendSeq`), the recall check and its remove request issued in the same tick (so a later send is ordered after it), un-steers serialized, their queue read bounded (5 s; a remove, once sent, is always awaited); a steer the runtime folds in before the remove answers settles normally, never as cancelled. Steer sends never wait on an un-steer. A change in the recallable set triggers an immediate heartbeat (`onCancellableChange`), so Cancel appears within ~1 s instead of up to 10 s. Live-verified twice: reported ~1 s after the send, row cancelled, never answered. At-most-once settle markers with the Claude retry ladder + heartbeat `settleFailed`. A pushed prompt the runtime opens after its turn's idle is adopted as its own turn (2 s grace), never a continuation. Tests: `copilot-sdk-steering-parity.test.mjs`, `copilot-steering-hold-chain.test.mjs` (real socket + relay ws service). |
