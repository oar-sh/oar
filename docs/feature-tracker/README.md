...
| Mid-turn steering (message into the live turn) | Partial — continuation turns only; delivered turns stay single-flight (SDK worker not opted into the link's steering gate; Claude-parity plan drafted 2026-09-25, see [copilot-sdk-worker.md](copilot-sdk-worker.md)) | Implemented — **unbounded per turn** ([claude-sdk.md](claude-sdk.md)); composer says "Steer", stopping lives on the bubbles | Not implemented (messages queue behind the turn) | Not implemented (messages queue behind the turn) |
...
| Conversation draft persistence + conflict checks | Implemented (version check actually enforced since 2026-09-24) | **Version-checked last-writer-wins.** `PATCH /api/conversation/:id/draft` rejects stale writes with 409 `draft-version-conflict` on a `baseDraftUpdatedAt` mismatch and returns the server draft (text + attachments). An explicit `null` base is conflict-checked only when the request carries `draftSyncVersion: 2` (this client and its pagehide outbox copy); a bare `null` from a pre-versioning tab stays unconditional. Until 2026-09-24 the check never ran: every keystroke nulled the local base, and the 3 s question poll / status paths also scheduled saves, so an idle device overwrote another's draft every few seconds. Client (`server/public/app/conversation-draft-sync.mjs` keeps per conversation the server draft the composer last agreed with; `conversation-view.js` → `persistConversationDraft`): saves happen only on real edits (input, attachments, emoji, explicit flushes on blur / switch / pagehide), poll and status paths call the button-only `syncComposerButtonState`, no-op saves are skipped, an unmodified composer adopts incoming drafts even while focused, and on 409 an unmodified composer adopts silently while a modified one wins and offers the other text back through a **Restore** toast (a swap, with **Undo**). While a send is in flight edit saves are held back and the post-send save carries whatever was typed; a pagehide copy taken mid-send asks for `acceptOwnSend` (passes only if this client's send wrote the newer version) and queued flushes older than five minutes are dropped (page and `sw.js`). Drafts are capped at 20,000 characters (`MAX_CONVERSATION_DRAFT_LENGTH`, mirrored by the client's `MAX_DRAFT_TEXT_LENGTH`), cut at a code-point boundary on both sides; the composer is never truncated. Tests: `conversation-draft-sync.test.mjs`, `sessions-routes-draft-patch.test.mjs`, `conversation-view.switch-guards.test.mjs`, e2e `tests/draft-sync-two-clients.spec.mjs`. |
...
| Queue claim ordering | Implemented | Eligible pending rows are claimed strictly in send order: `ORDER BY timestamp, rowid` across every `findPending*` variant (`server/repositories/message-repository.mjs`) and the owner-transfer scan. Neither `next_attempt_at` nor `retry_count` may rank rows: the old `CASE next_attempt_at` tier permanently ranked once-recovered rows behind every fresh arrival (live 2026-09-20: a steer-recovered row starved 5 minutes while newer messages jumped it), and ranking by `retry_count` ran a recovered older message after newer ones, since dead-worker recovery and `/api/requeue` bump it. Head-of-line protection is the `WHERE` clause's `next_attempt_at` backoff plus `MAX_REQUEUE_RETRIES` (terminal after N); `rowid` makes same-millisecond ties deterministic. |
| Conversation list title filter | Implemented | Sidebar filter input (debounced, case-insensitive title substring, NFC-normalized and folded with `toLocaleLowerCase` so decomposed accents match; multi-character folds like ß/ss are a known limit; × / Escape clears; no persistence). Because the list is paginated, an active filter drains the remaining pages through the infinite loader so it searches every conversation (`server/public/app/conversation-list-filter.mjs` → `drainRemainingPages`; busy loader attempts draw from a separate, larger budget than genuine no-progress retries, and a filter change restarts the drain; wiring in `journal-view.js`). A visually hidden `role="status"` live region announces the match count only when its text changes, saying "N loaded conversations match" when the drain gave up early. The input uses `max(16px, 1rem)` on coarse pointers / the mobile breakpoint so iOS does not zoom on focus. |
...
- 2026-09-24: **Steering audit fix wave** (pre-0.9.3). Claude steering: messages sent during a hold
  ...
  disabling, and cards are never bypassed; consumed steers settle at most once (worker settling set,
  `queue.consumed_at`, terminal `relay.steer-settle-failed` instead of a requeue); Stop suppresses the
  CLI's follow-up turn for already-pushed steers, which settle `stopped` with a one-tap **Resend**;
  folds get their own `folded` kind and replies anchor by `messages.source_message_id` (migration
  0004); the live-turn picker ranks by last output time. Draft sync: the version check now actually
  runs (see the draft row) — idle devices no longer overwrite other devices' drafts. Also: claim
  order strictly by send order including retried rows, sidebar filter polish, sticky live-bubble
  Stop, screenshot-only pending sends. Details: [claude-sdk.md](claude-sdk.md) steering row.
...
- 2026-09-20: **Multi-steering** — the per-turn steer cap on Claude conversations is gone
  ...
  a live turn plus draft, disabled while steering is held (superseded 2026-09-24: now "Queue";
  question card / plan approval, compaction,
