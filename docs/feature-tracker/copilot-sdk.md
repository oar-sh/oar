...
| `CopilotClient.deleteSession()` | Implemented | Conversation delete calls it through the SDK session importer's runtime client (`sdk-session-import-service.mjs` → `deleteSession`), after the conversation's session worker is stopped (`DELETE /api/conversation/:id`, `sessions-routes.mjs`). When the runtime cannot do it, the session goes on the SDK delete queue, which the extension engine (`.github/extensions/web-relay/polling/polling-loop.mjs`) still works off. |
