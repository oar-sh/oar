...
| `CopilotClient.deleteSession()` | Implemented | Conversation delete calls it through the SDK session importer's runtime client (`sdk-session-import-service.mjs` → `deleteSession`), after the conversation's session worker is stopped (`DELETE /api/conversation/:id`, `sessions-routes.mjs`). A conversation deleted before its first turn has no session folder yet and is not sent; a folder the runtime answers "Session file not found" for is removed directly. When the runtime cannot do it otherwise, the session goes on the SDK delete queue, which the extension engine (`.github/extensions/web-relay/polling/polling-loop.mjs`) still works off. |
| `session.disconnect()` | Implemented (import path) | Imported resumed sessions are torn down with `await resumed.disconnect()` — the actual 1.0.13 cleanup API; the old optional `stop?.()`/`dispose?.()` calls matched nothing (`server/services/sdk-session-import-service.mjs`). The joined extension session still gets no explicit disconnect (host-owned). |
