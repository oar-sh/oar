...
`@cursor/sdk` `^1.0.27`, then validated with a real API key: an SDK-level spike (results at the
...
| `AgentBusyError` recovery | Implemented (hardened 2026-08-16) | Two retries: close handle → recreate/resume; the second retry additionally passes `LocalSendOptions.force` (the SDK's documented recovery — expires the wedged persisted run). A third busy is terminal `cursor.agent_busy` (`cursor-turn-runner.mjs`). |
...
| Subagent lifecycle bubbles | Implemented **with text/thinking attribution** (2026-08-16) | Lifecycle from `tool_call` messages keyed by `call_id`; prose/thinking/nested-tool frames arrive as `tool-call-delta` updates whose `callId` is the same id (live-probed byte-equal), routed into the run's own stream/thought/activity channels. The pre-1.0.27 claim that the SDK exposes no parent-attribution field was wrong. One nesting level only (deeper levels dropped by the SDK at convert time). |
...
## Stability & recovery (added 2026-08-16)

| Mechanism | Notes / evidence |
| --------- | ---------------- |
| Stale-auth retry | Handles go stale ~4h idle while the API key stays valid. Budget: 2 recreate-and-retry attempts per turn with backoff, covering both the terminal-ERROR-result flavor and thrown `AuthenticationError`s (from `agent.send` or the handle rebuild). Before re-sending, the old abort signal is burned so a parked `ask_user` wait settles, and the failed attempt's still-open subagent runs are closed out. Handles idle > 30 min are recreated proactively (`cursor-turn-runner.mjs`). Rotating the API key in settings stops live Cursor workers so they respawn with the fresh key (`sessions-routes.mjs`). |
| Turn backstops | The merged stream has a stall watchdog (120s without SDK traffic fails the run as `cursor turn stalled`, deferring while an `ask_user` card is open); `readModelContextWindow` is raced against 8s and caches resolved-but-absent windows for 30 min so `composer-2.5`/`grok-4.5` stop paying a `models.list()` round trip per turn; the Stop control starts before `Agent.resume`/params setup so it is never inert (`cursor-sdk-adapter.mjs`, `cursor-turn-runner.mjs`). |
| Queue isolation contract | Cursor turns are born owned (`owner_sdk_session_id`); `findPendingForLegacyRelay` and `findPendingForWorker` both refuse session-worker-provider rows to other requesters (denylist derived from `shared/provider-routing.mjs`), `executed_provider` records who really answered — on success AND terminal-failure paths — and dead workers are detected in ~30s with owned rows recovered in place. |
