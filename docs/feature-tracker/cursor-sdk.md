...
| Context usage | Partial (by design) | Token totals only: the last `usage` event of a turn + the model's context window from `models.list()` are synthesized into a Claude-shaped snapshot (`cursor-context-usage.mjs` — occupancy = input+cacheRead+cacheWrite+output, categories deliberately empty) and posted to `POST /api/cursor-context-usage` into the shared `context_usage_json` column. No context-window-fill metric exists in the SDK. |
